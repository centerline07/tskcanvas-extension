...
# mastercanvas Browser Extension

> Save all open browser tabs as a new tree in mastercanvas with one click.
>
> **Move this file** to the `mastercanvas-extension` repo as `README.md` or `docs/README.md`
...
2. Authenticates with the same Clerk app as mastercanvas.app (via Sync Host)
...
npm create plasmo@latest mastercanvas-extension -- --with-clerk
cd mastercanvas-extension
...
**Best UX**: Enable Sync Host so users sign in once on mastercanvas.app and the extension automatically picks up their session.
...
clerk.addListener(({ user }) => {
  console.log('[mastercanvas] Auth state:', user ? 'signed in' : 'signed out');
});
```
...
function SignedOutContent() {
  ...
  return (
    <div className="p-4 w-80">
      <h2 className="font-semibold mb-2">Sign in to mastercanvas</h2>
      ...
      <p className="text-xs text-gray-400 mt-3 text-center">
        Tip: Sign in at mastercanvas.app for automatic sync
      </p>
    </div>
  );
}
...
- [mastercanvas API Docs](./browser-extension-api.md)
